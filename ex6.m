...
for i = 1:2:M-1
    for j = 1:2:N-1
        ...
        if j < N/2
            % Primer mitad de la imagen: usar kernel [0,1;1,0]
            imagen_resultante(i, j, 1) = Rw(i, j+1); % 0
            imagen_resultante(i, j+1, 1) = R(i, j);  % 1
            imagen_resultante(i+1, j, 1) = R(i+1, j); % 1
            imagen_resultante(i+1, j+1, 1) = Rw(i+1, j+1); % 0

            imagen_resultante(i, j, 2) = Gw(i, j+1); % 0
            imagen_resultante(i, j+1, 2) = G(i, j);  % 1
            imagen_resultante(i+1, j, 2) = G(i+1, j); % 1
            imagen_resultante(i+1, j+1, 2) = Gw(i+1, j+1); % 0

            imagen_resultante(i, j, 3) = Bw(i, j+1); % 0
            imagen_resultante(i, j+1, 3) = B(i, j);  % 1
            imagen_resultante(i+1, j, 3) = B(i+1, j); % 1
            imagen_resultante(i+1, j+1, 3) = Bw(i+1, j+1); % 0
        else
            % Segunda mitad de la imagen: usar kernel [1,0;0,1]
...
title('Imagen resultante con bloques 2x2 combinados y patrón invertido en la primera mitad');
